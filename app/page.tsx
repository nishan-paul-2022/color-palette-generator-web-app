...
function SortableSegment({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`w-full border rounded-md mb-3 overflow-hidden ${isSelected ? "ring-2 ring-primary" : ""}`}
    ...
    >
      <div className="flex items-stretch">
        {selectionMode ? (
          ...
          <div className="p-4 cursor-grab bg-background" {...attributes} {...listeners}>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    style={{
                      color: textColor,
                      textShadow: `0 1px 2px ${textColor === "#ffffff" ? "rgba(0,0,0,0.5)" : "rgba(255,255,255,0.5)"}`,
                    }}
                    onClick={(e) => {
                      e.stopPropagation()
...
export default function ColorSegmentGenerator() {
  ...
  const [exportOrientation, setExportOrientation] = useState<"horizontal" | "vertical">("horizontal")
  ...
  useEffect(() => {
    const savedSets = localStorage.getItem("colorPalateSets")
    if (savedSets) {
      try {
    ...
    const savedActiveSetId = localStorage.getItem("colorPalateActiveSetId")
    if (savedActiveSetId) {
  ...
  useEffect(() => {
    localStorage.setItem("colorPalateSets", JSON.stringify(sets))
    localStorage.setItem("colorPalateActiveSetId", activeSetId)
  }, [sets, activeSetId])
  ...
  const addOrUpdateSegment = () => {
    if (newColor && newTitle) {
      if (editingId) {
            ...
            set.id === activeSetId
              ? {
                  ...
                  segments: set.segments.map((segment) =>
                    segment.id === editingId ? { ...segment, color: newColor, title: newTitle } : segment,
                  ),
                }
              : set,
          ),
        ...
        setSets(sets.map((set) => (set.id === activeSetId ? { ...set, segments: [...set.segments, newSegment] } : set)))
      }
  ...
  const handleDelete = (id: string) => {
    setSets(
      sets.map((set) =>
        set.id === activeSetId ? { ...set, segments: set.segments.filter((segment) => segment.id !== id) } : set,
      ),
    )
  ...
  const deleteSelected = () => {
    setSets(
      sets.map((set) =>
        set.id === activeSetId
          ? { ...set, segments: set.segments.filter((segment) => !selectedSegments.includes(segment.id)) }
          : set,
      ),
    )
    setSelectedSegments([])
  ...
  const updateSetName = () => {
    if (editingSetId && newSetName.trim()) {
      setSets(sets.map((set) => (set.id === editingSetId ? { ...set, name: newSetName.trim() } : set)))
  ...
  const exportAsImage = () => {
    ...
    const segmentsToExport =
      selectedSegments.length > 0 ? segments.filter((seg) => selectedSegments.includes(seg.id)) : segments
    ...
    segmentsToExport.forEach((segment, index) => {
      // Calculate position
      const x = exportOrientation === "horizontal" ? startX + index * segmentWidth : startX
      const y = exportOrientation === "horizontal" ? startY : startY + index * segmentHeight
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <h1 className="text-2xl font-bold mb-6">Color Palate Generator</h1>
      ...
      <div className="flex flex-col gap-6">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-medium">Color Palates</h2>
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
              ...
              {sets.map((set) => (
                <div
                  ...
                  className={`relative group border rounded-md px-3 py-2 cursor-pointer ${
                    set.id === activeSetId ? "bg-primary/10 border-primary" : "hover:bg-muted"
                  }`}
                  onClick={() => setActiveSetId(set.id)}
                >
                  <span className="font-medium">{set.name}</span>
                  <span className="ml-2 text-xs text-muted-foreground">({set.segments.length})</span>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => openEditSetDialog(set.id)}>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-lg font-medium text-primary">Create New Segment</CardTitle>
            <p className="text-sm text-muted-foreground">Adding to "{activeSet.name}"</p>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="flex flex-col gap-4">
              <div className="w-full rounded-md overflow-hidden border relative" style={{ backgroundColor: newColor }}>
        ...
        {segments.length > 0 && (
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-lg font-medium text-primary">Your Color Collection</CardTitle>
              <p className="text-sm text-muted-foreground">Viewing "{activeSet.name}"</p>
              <div className="flex gap-2">
            ...
            {selectionMode && selectedSegments.length > 0 && (
              <div className="px-6 py-2 bg-muted/20 border-y flex items-center justify-between">
                <div className="text-sm">
                  {selectedSegments.length} {selectedSegments.length === 1 ? "segment" : "segments"} selected
                </div>
                <div className="flex gap-2">
      ...
      <Dialog open={showSetDialog} onOpenChange={setShowSetDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={editingSetId ? updateSetName : addNewSet}>{editingSetId ? "Update" : "Create"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showExportDialog} onOpenChange={setShowExportDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Configure how you want to export your color segments as an image.</DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-4">
                <Button
                  type="button"
                  variant={exportOrientation === "horizontal" ? "default" : "outline"}
                ...
                <Button
                  type="button"
                  variant={exportOrientation === "vertical" ? "default" : "outline"}
